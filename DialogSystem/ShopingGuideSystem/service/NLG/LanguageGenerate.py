...
class NaturalLanguageGenerate:
    ...
    def GetAnswer(self, product=None, strategy=None, query="Hello"):
        """
        获取语言生成的回复，对外接口
        :param
        ...
        if product == "空调":
            ...
            self.answer = self.GetAnswerChat(query)
        return self.answer
    ...
    def GetAnswerListVocabulary(self):
        ...
        television = []
        electriccooker = []
        with open("ShopingGuideSystem/service/NLG/Vocabulary/空调.txt", "r") as f:
            for line in f:
                airconditioning.append(line.split()[0])
        with open("ShopingGuideSystem/service/NLG/Vocabulary/冰箱.txt", "r") as f:
            for line in f:
                refrigerator.append(line.split()[0])
        with open("ShopingGuideSystem/service/NLG/Vocabulary/洗衣机.txt", "r") as f:
            for line in f:
                washingmachine.append(line.split()[0])
        with open("ShopingGuideSystem/service/NLG/Vocabulary/电视.txt", "r") as f:
            for line in f:
                television.append(line.split()[0])
        with open("ShopingGuideSystem/service/NLG/Vocabulary/电饭煲.txt", "r") as f:
            for line in f:
                electriccooker.append(line.split()[0])
        return airconditioning, refrigerator, washingmachine, television, electriccooker

    def GetAnswerChat(self, query):
        """
        对于闲聊意图的回复
        目前就问候、结束进行特殊模板回复
        其余闲聊统一回复"对不起，我还不能理解此问句"
        :param query:
