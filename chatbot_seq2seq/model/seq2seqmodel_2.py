...
def lstm_cell(dropout_keep_prob, reuse=False):
    lstm_cell = tf.nn.rnn_cell.LSTMCell(size_layer, reuse, activation=tf.nn.tanh)
...
with train_graph.as_default():
    ...
    with tf.variable_scope("encoder"):
        ...
        if bidirection:
            encoder_cell_bw = tf.nn.rnn_cell.MultiRNNCell([lstm_cell(dropout_keep_prob) for _ in range(num_layers)])
            (
                (encoder_fw_outputs, encoder_bw_outputs),
                (encoder_fw_state, encoder_bw_state)
            ) = tf.nn.bidirectional_dynamic_rnn(
                cell_bw=encoder_cell_bw,
                cell_fw=encoder_cells,
        ...
        for epoch_i in range(1, epochs + 1):
            for batch_i, (targets_batch, sources_batch, targets_lengths, sources_lengths) in enumerate(
                ...
                if step % display_step == 0:
                    ...
                    error = 0.0
                    vali_loss = []
                    val_data = random_choise(test_source, 2000)
