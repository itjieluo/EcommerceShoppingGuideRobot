from SLU.PurposeRecognise import Purpose
from SLU.SlotExtract import Slot
from SLU.Models.SLUModelInterface import SLUModel
from DPO.Strategy import ActStrategy
from DST.State import State
from NLG.LanguageGenerate import NaturalLanguageGenerate
...
class CentralControl:
    ...
    def Sess(self, query):
        ...
        if self.state_now[0] == "": #初始化状态时
            self.purpose_now = self.purpose.GetPurpose(query, self.slu_model) #进行购买商品意图识别
...
#----策略第一版入库------2版打
                ...
                self.product_now = "空调"
                self.slot_now = self.slot.GetSlot(query, self.product_now, self.slu_model) #进行槽位提取
            ...
            self.purpose_now = self.purpose.IsMult(query, self.product_now, self.slu_model) #对query进行多轮意图识别
            if self.purpose_now == 0: #如果不含多轮意图，重复初始化状态操作
                ...
                self.slot_new = self.slot.GetSlot(query, self.product_now, self.slu_model) #进行新槽位提取
