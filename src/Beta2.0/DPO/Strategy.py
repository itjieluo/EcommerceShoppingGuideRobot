...
class ActStrategy:
    ...
    def GetStrategy(self, product, slot, suibian = False):
        """
        获取动作策略结果，对外接口
        :param
    ...
    def GetSuggestSlotVocabulary(self):
        ...
        television = []
        electriccooker = []
        with open("./DPO/Vocabulary/空调.txt", "r") as f:
            for line in f:
                airconditioning.append(line.split()[0])
        with open("./DPO/Vocabulary/冰箱.txt", "r") as f:
            for line in f:
                refrigerator.append(line.split()[0])
        with open("./DPO/Vocabulary/洗衣机.txt", "r") as f:
            for line in f:
                washingmachine.append(line.split()[0])
        with open("./DPO/Vocabulary/电视.txt", "r") as f:
            for line in f:
                television.append(line.split()[0])
        with open("./DPO/Vocabulary/电饭煲.txt", "r") as f:
            for line in f:
