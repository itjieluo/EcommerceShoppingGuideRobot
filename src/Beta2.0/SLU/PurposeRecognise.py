...
class Purpose:
    ...
    def GetPurpose(self, query, model):
        """
        ...
        """
        self.purpose_model = model.RunPursoleModel(query)
        if self.purpose_model == [0]:
    ...
    def IsMult(self, query, product, model = None):
        """
        ...
        """
        if product == "空调":
            self.slot_input, self.slot_ismult = model.RunSlotModel(query)
            self.ismult = False
            for i in self.slot_ismult:
                if i != "O":
                    self.ismult = True
        else:
    ...
    def GetProductVocabulary(self):
        ...
        """
        product = []
        with open("./SLU/Vocabulary/product.txt", "r") as f:
            for line in f:
    ...
    def GetSlotVocabulary(self):
        ...
        television = []
        electriccooker = []
        with open("./SLU/Vocabulary/空调.txt", "r") as f:
            for line in f:
                airconditioning.append(line.split()[0])
        with open("./SLU/Vocabulary/冰箱.txt", "r") as f:
            for line in f:
                refrigerator.append(line.split()[0])
        with open("./SLU/Vocabulary/洗衣机.txt", "r") as f:
            for line in f:
                washingmachine.append(line.split()[0])
        with open("./SLU/Vocabulary/电视.txt", "r") as f:
            for line in f:
                television.append(line.split()[0])
        with open("./SLU/Vocabulary/电饭煲.txt", "r") as f:
            for line in f:
                electriccooker.append(line.split()[0])
        return airconditioning, refrigerator, washingmachine, television, electriccooker

# from Models.SLUModelInterface import SLUModel
#
# model = SLUModel()
# a = Purpose()
# a.Init()
# b = a.GetPurpose("我要买电饭煲锤子", model)
# print(b)
# c = a.IsMult("我要一个海尔的", "空调", model)
# print(c)
# d = a.IsMult("我要一个", "空调", model)
# print(d)
